...
`;
            ...
            timerInterval = setInterval(() => {
                ...
                if (timeLeft === 0) {
                    ...
                    if ((questionNo + 1) > q.length) {
                        ...
                        $($('#x-option').parent().find('label')).html(q[questionNo].C[4]);
                        $($('#x-option').parent().find('label')).html(q[questionNo].C[5]);
                        $($('#x-option').parent().find('label')).html(q[questionNo].C[6]);
                        $($('#x-option').parent().find('label')).html(q[questionNo].C[7]);
                        $($('#x-option').parent().find('label')).html(q[questionNo].C[8]);
                    }
                }
    ...
    var q = [
        ...
        { 'Q': 'Who is the first Indian woman to win as Asian Games gold in 400m run?', 'A': 2, 'C': ['M.L. Valsamma', 'Kamaljit Sandhu', 'PT Usha', 'K Malleshwari'] },
        {
            'Q': 'The terms Volley, Smash, Service are related to which among the following sports?', 'A': 2, 'C': ['Volleyball', 'Lawn Tennis', 'Table Tennis', 'Badminton']
        },
        { 'Q': 'The famous football player Maradona belongs to which among the following countries?', 'A': 3, 'C': ['Brazil', 'Chile', 'Argentina', 'Italy'] },
        { 'Q': 'Who among the following served as India’s first Test Captain?', 'A': 3, 'C': ['Nawab of Pataudi Senior', 'Nawab Mansoor Ali Khan', 'C. K. Nayudu', 'Lala Amarnath'] },
        { 'Q': 'Who among the following was first Rajiv Gandhi Khel Ratna?', 'A': 1, 'C': ['Viswanathan Anand', 'Geet Sethi', 'Karnam Malleswari', 'Nameirakpam Kunjarani'] },
        { 'Q': 'In which city was the finals of 2015 ICC World Cup played?', 'A': 3, 'C': ['Brisbane', 'Sydney', 'Melbourne', 'Auckland'] }
    ...
    function updateProgress() {
        ...
        progressPercentage = Math.round((questionNo / 9) * 100);
        ...
        setTimeout(function () {
            ...
            if ((questionNo + 1) > q.length) {
                ...
                setTimeout(function () {
                    ...
                    $('#scoreModal').modal('show');
                    $('#quizResult').html(toAppend);
                    $('#totalCorrect').html(correctCount * 9);
                    $('#quizResult').show();
                    $('#loadbar').fadeOut();
                    $('#result-of-question').show();
                    $('#graph-result').show();
